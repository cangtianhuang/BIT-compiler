...
public class CVisitor extends lab7BaseVisitor<String> {
	...
	// CQuats列表，储存每一scope中的四元式，未启用
	private ArrayList<CQuat> scopeCQuats;
	// 内部内存，保留未使用
	...
	public String visitInit_declarator_decl_init(lab7Parser.Init_declarator_decl_initContext ctx) {
		...
		if (!preDefFlag) {
			...
			} else CQuats.add(new CQuat("=", expr, null, tempInfo.id));
		}
		return null;
	}
	...
	public String visitCompound_statement(lab7Parser.Compound_statementContext ctx) {
		...
		//CQuats.add(new CQuat("compoundBegin", null, null, "cs" + memoryEnv.peek()));
		preDefFlag = true;
		...
		//CQuats.add(new CQuat("compoundEnd", null, null, "cs" + memoryEnv.peek()));
		memoryEnv.pop();
		return null;
	}
	...
	public String visitLabel_id(lab7Parser.Label_idContext ctx) {
		...
		if (jumpTable.get(token.getText()) != null) {
			...
			if (labelInfo.defFlag == 1)
				...
				labelInfo.defFlag = 1;
				int i = CQuats.size();
				for (String s = String.valueOf(labelInfo.add); s != null; ) {
					CQuat cQuat = CQuats.get(Integer.parseInt(s));
					s = cQuat.getRes();
					cQuat.setRes(String.valueOf(i));
				}
				labelInfo.add = i;
	...
	public String visitSelect_switch(lab7Parser.Select_switchContext ctx) {
		...
		for (String s : jumpTable.keySet()) {
			if (s.startsWith("case:")) {
				LabelInfo labelInfo = jumpTable.get(s);
				CQuats.add(new CQuat("je", cond, s.substring(5), String.valueOf(labelInfo.add)));
				jumpTable.remove(s);
			} else if (s.startsWith("def:")) {
				LabelInfo labelInfo = jumpTable.get(s);
				CQuats.add(new CQuat("j", null, null, String.valueOf(labelInfo.add)));
				jumpTable.remove(s);
			}
		}
		env.setEnd();
